...
**§6.5 — The Limaçon Family and the Cardioid Threshold (2026-08-16) `[MEASURED]`:** The *shape* of the curve that produces the zeros, measured rather than asserted. ζ(½+it) sampled at 6000 points over t ∈ [0.05, 50]: **9 of 10 near-origin passages match known Riemann zeros to three decimals.** Fitting each loop between consecutive passages to the limaçon family `r = a + b·cos(θ−φ)` with free phase gives **R² between 0.86 and 1.0000** — the shape family is right. But **0 of 9 loops fall in 0.85 < |b/a| < 1.15**; every loop has |b/a| > 1. Since |b/a| = 1 *is* the cardioid, and it is exactly the boundary between a loop that merely touches the origin and one that crosses it, **the cardioid is the sifting threshold, not the curve**: |b/a| < 1 produces no zero, |b/a| = 1 touches, |b/a| > 1 crosses and yields a zero. Every loop ζ traces lies strictly past it, which is why zeros exist at all. The same marginal condition recurs in four unrelated structures — limaçon |b/a| = 1, Mandelbrot |multiplier| = 1, PSL(2,ℤ) |trace| = 2, sedenion orbit growth/step = 1.000000 — each the line between closing and escaping. **Correction on the record:** an earlier conversational note claiming the curve *is* a cardioid is false as written (global cardioid fit gives R² = −0.7722, worse than a constant; the curve winds 13.36 times while a cardioid is single-valued over one turn). Full account: [PAPER.md §6.5](PAPER.md).
